...
if ( ! class_exists( 'Subscribe' ) ) {
	...
	class Subscribe extends Notice {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'wp_ajax_jltelmm_subscribe', array( $this, 'jltelmm_subscribe' ) );
		}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltelmm_subscribe() {
			check_ajax_referer( 'jltelmm_subscribe_nonce' );
		...
		public function notice_content() {
			...
			?>
			<div class="notice notice-jltelmm is-dismissible notice-plugin-review notice-<?php echo esc_attr( $this->color ); ?> jltelmm-notice-<?php echo esc_attr( $this->get_id() ); ?>">
				<button type="button" class="notice-dismiss jltelmm-notice-dismiss"></button>
				<img width="70" height="70" src="<?php echo esc_url( JLTELMM_IMAGES . '/logo.png' ); ?>" alt="<?php esc_attr_e( 'Logo', 'master-mega-menu-lite' ); ?>">
				<div class="jltelmm-subscribe-content">
					<?php $this->set_title(); ?>
					<form style="display:flex">
						<div class="jltelmm-plugin-subscribe-input">
						...
						<button type="submit" class="button button-primary jltelmm-subscribe-btn"><?php esc_html_e( 'Get Discount', 'master-mega-menu-lite' ); ?></button>
					</form>
				</div>
			</div>
			<?php
		...
		public function core_script( $trigger_time ) {
			...
			<script>
				jQuery(document).on('submit', '.jltelmm-notice-subscribe .jltelmm-subscribe-content form', function(e) {
					...
					let formWrapper = form.closest('.jltelmm-subscribe-content');
					...
					jQuery.ajax({
							...
							crossDomain: true,
							data: {
								action: 'jltelmm_subscribe',
								_wpnonce: '<?php echo esc_js( wp_create_nonce( 'jltelmm_subscribe_nonce' ) ); ?>',
								name: name,
								email: email,
							}
						})
						.done(function(response) {
							formWrapper.hide().after('<p class="jltelmm--notice-message"><strong>' + response.data + '</strong><p>');
							let subsTimeout = setTimeout(function() {
								jltelmm_notice_action(null, form, 'disable');
