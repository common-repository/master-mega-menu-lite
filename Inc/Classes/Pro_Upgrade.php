...
class Pro_Upgrade {
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jltelmm_dashboard_widget',                          // Widget slug.
			esc_html__( 'master-mega-menu-lite News & Updates', 'master-mega-menu-lite' ), // Title.
		...
		$example_widget_backup = array( 'jltelmm_dashboard_widget' => $default_dashboard['jltelmm_dashboard_widget'] );
		unset( $default_dashboard['jltelmm_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="jltelmm-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="jltelmm-dashboard-promo" style="--jltelmm-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="master-mega-menu-lite Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="jltelmm-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				$item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . __( 'ago', 'master-mega-menu-lite' );
		...
		<div class="jltelmm-dashboard_footer">
			<ul>
				<li class="jltelmm-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'master-mega-menu-lite' ); ?>
                        </span>
                        <span aria-hidden="true" class="dashicons dashicons-external"></span>
                    </a>
                </li>
                <li class="jltelmm-overview__help">
                ...
                <li class="jltelmm-overview__upgrade">
		...
		<style>
			...
			.jltelmm-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.jltelmm-rss-widget .jltelmm-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.jltelmm-rss-widget .jltelmm-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.jltelmm-rss-widget .jltelmm-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.jltelmm-rss-widget .jltelmm-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.jltelmm-rss-widget .jltelmm-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jltelmm_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jltelmm_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jltelmm_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( JLTELMM_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jltelmm_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jltelmm_sheet_promo_data', $remote_data );
			update_option( 'jltelmm_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jltelmm_sheet_promo_data_reset' );
		}
	}
	...
	public function get_sheet_promo_remote_data() {
		...
		$plugin_slug = Helper::jltelmm_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="jltelmm-popup jltelmm-upgrade-popup" id="jltelmm-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="jltelmm-popup-overlay"></div>

			<div class="jltelmm-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --jltelmm-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="jltelmm-popup-modal-footer">
					...
					<div class="jltelmm-popup-countdown" style="display: none;">
						<span class="jltelmm-popup-countdown-text">Deal Ends In</span>
					...
					<!-- button  -->
					<a class="jltelmm-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#jltelmm-popup'),
				plugin_data = <?php echo json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".jltelmm-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".jltelmm-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".jltelmm-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".jltelmm-popup-countdown").hide(0);
				}
