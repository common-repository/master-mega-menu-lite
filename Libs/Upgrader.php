...
use JLTELMM\JLE_Elementor_Mega_Menu;
...
if ( ! class_exists( 'Upgrader' ) ) {
	...
	class Upgrader {
		...
		public function __construct() {
			$this->option_name = JLE_Elementor_Mega_Menu::plugin_version_key();
		}
		...
		protected function get_installed_version() {
			return get_option( $this->option_name, JLTELMM_VER );
		}
		...
		public function if_updates_available() {
			if ( version_compare( $this->get_installed_version(), JLTELMM_VER, '<' ) ) {
				return true;
			}
		...
		public function run_updates() {
			...
			$path              = trailingslashit( JLTELMM_DIR );
