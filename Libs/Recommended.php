...
if ( ! class_exists( 'Recommended' ) ) {
	...
	class Recommended {
		...
		public function __construct( $menu_order = 99 ) {
			...
			add_action( 'wp_ajax_jltelmm_recommended_upgrade_plugin', array( $this, 'jltelmm_recommended_upgrade_plugin' ) );
			add_action( 'wp_ajax_jltelmm_recommended_activate_plugin', array( $this, 'jltelmm_recommended_activate_plugin' ) );
		}
		...
		public function render_recommended_plugins() { ?>
			<div class='jltelmm-recommended-wrapper'>
				<?php $this->header(); ?>
				<?php $this->body(); ?>
			</div>
			<?php
		...
		public function header() {
			?>
			<div class='jltelmm-recommended-header'>
				<div class='jltelmm-recommended-title'>
					<h2>
						<?php echo esc_html__( 'Recommended Plugins', 'master-mega-menu-lite' ); ?>
					</h2>
				</div>
				<div class='jltelmm-recommended-menu'>
					<div class="wp-filter">
						...
						<form class="search-form jltelmm-search-plugins mr-0" method="get">
							<input type="hidden" name="tab" value="search">
							<label class="screen-reader-text" for="search-plugins">
								<?php echo esc_html__( 'Search Plugins', 'master-mega-menu-lite' ); ?>
							</label>
							<input type="search" name="s" id="search-plugins" value="" class="wp-filter-search" placeholder="<?php echo esc_html__( 'Search plugins...', 'master-mega-menu-lite' ); ?>">
							<input type="submit" id="search-submit" class="button hide-if-js" value="<?php echo esc_html__( 'Search Plugins', 'master-mega-menu-lite' ); ?>">
						</form>
					</div>
				</div>
			</div>
					...
					<div class="plugin-card-bottom">
						<div class="column-downloaded">
							<span class="plugin-status">
								<?php
								echo esc_html__( 'Status:', 'master-mega-menu-lite' );

								if ( 'install' === $install_status['status'] ) {
									?>
									<span class="plugin-status-not-install" data-plugin-url="<?php echo esc_attr( $plugin['download_link'] ); ?>"><?php echo esc_html__( 'No Installed', 'master-mega-menu-lite' ); ?></span>
									<?php
								} elseif ( 'update_available' === $install_status['status'] ) {
									if ( is_plugin_active( $install_status['file'] ) ) {
										?>
										<span class="plugin-status-active">
											<?php echo esc_html__( 'Active', 'master-mega-menu-lite' ); ?>
										</span>
										<?php
									} else {
										?>
										<span class="plugin-status-inactive" data-plugin-file="<?php echo esc_attr( esc_attr( $install_status['file'] ) ); ?>">
											<?php echo esc_html__( 'Inactive', 'master-mega-menu-lite' ); ?>
										</span>
										<?php
									}
								...
								if ( 'install' === $install_status['status'] ) {
									...
									<li class="mr-0">
										<button class="install-now button button-primary" data-install-url="<?php echo esc_attr( $plugin['download_link'] ); ?>">
											<?php echo esc_html__( 'Install Now', 'master-mega-menu-lite' ); ?>
										</button>
									</li>
									<?php
								} elseif ( 'update_available' === $install_status['status'] ) {
									...
									<li class="mr-0">
										<button class="update-now button" data-plugin="<?php echo esc_attr( $install_status['file'] ); ?>" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-update-url="<?php echo esc_attr( $install_status['url'] ); ?>">
											<?php echo esc_html__( 'Update Now', 'master-mega-menu-lite' ); ?>
										</button>
									</li>
									<?php
								} elseif ( ( 'latest_installed' === $install_status['status'] ) || ( 'newer_installed' === $install_status['status'] ) ) {
									if ( is_plugin_active( $install_status['file'] ) ) {
										...
										<li class="mr-0">
											<button type="button" class="button button-disabled" disabled="disabled">
												<?php echo esc_html__( 'Activated', 'master-mega-menu-lite' ); ?>
											</button>
										</li>
										<?php
									} elseif ( current_user_can( 'activate_plugin', $install_status['file'] ) ) {
										...
										<button class="button activate-now" data-plugin-file="<?php echo esc_attr( $install_status['file'] ); ?>">
											<?php echo esc_html__( 'Activate', 'master-mega-menu-lite' ); ?>
										</button>
										<?php
									} else {
										?>
										<li class="mr-0">
											<button type="button" class="button button-disabled" disabled="disabled">
												<?php echo esc_html__( 'Installed', 'master-mega-menu-lite' ); ?>
											</button>
										</li>
										<?php
									}
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltelmm_recommended_activate_plugin() {
			try {
				if ( isset( $_POST['file'] ) ) {
					$nonce = isset( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';

					if ( ! wp_verify_nonce( $nonce, 'jltelmm_recommended_nonce' ) ) {
						wp_send_json_error( array( 'mess' => __( 'Nonce is invalid', 'master-mega-menu-lite' ) ) );
					}
					...
					wp_send_json_success(
						array(
							'mess' => __( 'Activate success', 'master-mega-menu-lite' ),
						)
					);
				}
			} catch ( \Exception $ex ) {
				wp_send_json_error(
					array(
						'mess' => __( 'Error exception.', 'master-mega-menu-lite' ),
						array(
							'error' => $ex,
						),
					)
				...
				wp_send_json_error(
					array(
						'mess' => __( 'Error.', 'master-mega-menu-lite' ),
						array(
							'error' => $ex,
						),
					)
		 ...
		 * @author Jewel Theme <support@jeweltheme.com>
		 */
		public function jltelmm_recommended_upgrade_plugin() {
			try {
				require_once ABSPATH . 'wp-admin/includes/plugin-install.php';
					...
					if ( 'install' === $type ) {
						...
						if ( $plugin_data && ! is_wp_error( $plugin_data ) ) {
							...
							if ( is_wp_error( $active_plugin ) ) {
								...
								wp_send_json_success(
									array(
										'mess' => __( 'Install success', 'master-mega-menu-lite' ),
									)
								);
							}
						} else {
						...
						if ( is_wp_error( $result ) ) {
							...
							wp_send_json_success(
								array(
									'mess'   => __( 'Update success', 'master-mega-menu-lite' ),
									'active' => $is_active,
								)
							);
						}
