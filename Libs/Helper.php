...
if ( ! class_exists( 'Helper' ) ) {
	...
	class Helper {
		 ...
		 */
		public static function jltelmm_slug_cleanup() {
			return str_replace( '-', '_', strtolower( JLTELMM_SLUG ) );
		}
		 ...
		 */
		public static function jltelmm_current_datetime() {
			if ( function_exists( 'current_datetime' ) ) {
				return current_datetime();
			}

			return new \DateTimeImmutable( 'now', self::jltelmm_wp_timezone() );
		}

		/**
		 * Function jltelmm_wp_timezone() compability for wp version < 5.3
		 *
		 * @return DateTimeZone
		 */
		public static function jltelmm_wp_timezone() {
			if ( function_exists( 'wp_timezone' ) ) {
				return wp_timezone();
			}

			return new \DateTimeZone( self::jltelmm_wp_timezone_string() );
		}
		...
		public static function api_endpoint() {
			...
			$api_endpoint     = apply_filters( 'jltelmm_endpoint', $api_endpoint_url );
		...
		public static function crm_endpoint() {
			...
			$crm_endpoint     = apply_filters( 'jltelmm_crm_crm_endpoint', $crm_endpoint_url );
		...
		public static function crm_survey_endpoint() {
			...
			$crm_feedback_endpoint     = apply_filters( 'jltelmm_crm_crm_endpoint', $crm_feedback_endpoint_url );

			return trailingslashit( $crm_feedback_endpoint );
		}

		/**
		 * Function jltelmm_wp_timezone_string() compability for wp version < 5.3
		 *
		 * @return string
		 */
		public static function jltelmm_wp_timezone_string() {
		...
		public static function get_merged_data( $data, $start_date = '', $end_data = '' ) {
			$_data = shortcode_atts(
				array(
					'image_url'        => JLTELMM_IMAGES . '/promo-image.png',
			...
			if ( empty( $_data['image_url'] ) ) {
				$_data['image_url'] = JLTELMM_IMAGES . '/promo-image.png';
			}
