(function($) {
    ...
    // Notice Hide
    $('body').on('click', '.jltelmm-upgrade-popup .popup-dismiss', function(evt) {
        evt.preventDefault();
        $(this).closest('.jltelmm-upgrade-popup').fadeOut(200);
    });

    // Notice Show
    $('body').on('click', '.disabled', function(evt) {
        evt.preventDefault();
        $('.jltelmm-upgrade-popup').fadeIn(200);
    });
    ...
    // Search
    $('.jltelmm-search-plugins #search-plugins').on('keyup', function() {
        var value = $(this).val();
...
function activatePlugin(element, file) {
    ...
    jQuery.ajax({
        url: JLTELMMCORE.admin_ajax,
        type: "POST",
        data: {
            action: "jltelmm_recommended_activate_plugin",
            file: file,
            nonce: JLTELMMCORE.recommended_nonce
        },
...
function installPlugin(element, plugin) {
    ...
    jQuery.ajax({
        ...
        type: "POST",
        data: {
            action: "jltelmm_recommended_upgrade_plugin",
            type: 'install',
            plugin: plugin,
            nonce: JLTELMMCORE.recommended_nonce
        },
